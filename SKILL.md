...
2. **系统资源检测**：自动检测 CPU/内存/GPU，推荐合适模型
3. **语音转文字**：使用 Whisper/Vosk 模型进行高精度语音识别
4. **国内镜像支持**：自动使用国内镜像源，失败时自动切换
5. **错误处理**：智能降级策略，确保转录成功
6. **批量处理**：支持批量处理多个 B 站视频
7. **大会员支持**：支持扫码登录获取大会员权限

### 🔧 技术特点
- **优先级优化**：字幕优先，避免不必要的下载和转录（节省 90% 时间）
- **智能模型选择**：根据系统资源和视频时长自动选择最合适模型
- **多模型支持**：Whisper tiny/base/small/medium 或 Vosk 离线模型
- **视频时长策略**：
  - < 10 分钟：Vosk 离线（快速、免费）
  - 10-20 分钟：Vosk 或 Whisper（可选）
  - > 20 分钟：Whisper 在线（强制，避免超时）
- **Cookie 管理**：支持扫码登录和 Cookie 文件管理
- **进度显示**：实时显示处理进度和使用方法
- **结果验证**：自动验证转录内容与视频标题相关性
- **镜像自动切换**：HuggingFace 失败时自动切换到国内镜像
